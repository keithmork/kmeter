...
# 画曲线，点的x坐标, y坐标
# 颜色：r 红 g 绿 b 蓝，点：o 圆 s 方 ^ 三角，线：-- 折线，合起来 'ro--' 'gs--' 'b^--' 等
plt.plot(durations_in_second, average_times, label='Avg. Response Time (ms)')
plt.plot(durations_in_second, throughputs, label='Throughput (requests/s)')
# plt.plot(durations_in_second, min_times, label='Min Response time (ms)')
# plt.plot(durations_in_second, max_times, label='Max Response time (ms)')
# plt.plot(durations_in_second, request_counts, label='Request Count')
# plt.plot(durations_in_second, error_counts, label='Error Count')
